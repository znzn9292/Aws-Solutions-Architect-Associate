...
- **정의**: 필요할 때마다 인스턴스를 생성하고, 사용한 만큼만 비용을 지불하는 방식.
- **특징**:
  - 장기 약정이나 선결제가 필요 없음.
  - 필요할 때 언제든지 인스턴스를 시작하고 중지 가능.
  - 초기 비용 없이 시간당 요금 부과.
- **사용 사례**:
  - 예측 불가능한 워크로드.
  - 애플리케이션 개발 및 테스트 환경.
  - 스파이크가 있거나 간헐적인 트래픽을 처리해야 하는 경우.

### 2. 스팟 인스턴스 (Spot Instances)
- **정의**: AWS의 여유 컴퓨팅 용량을 최대 90% 저렴한 가격에 사용하는 방식.
- **특징**:
  - 인스턴스가 언제든지 AWS에 의해 종료될 수 있음.
  - 지정한 최대 가격으로 입찰하여 사용 가능.
  - 사용 가능 시 가장 저렴한 인스턴스.
- **사용 사례**:
  - 유연한 시작 및 종료가 가능한 작업.
  - 빅 데이터 분석, 배치 작업, 데이터 처리가 많은 워크로드.
  - 기계 학습, 테스트 및 개발 환경.

### 3. 예약 인스턴스 (Reserved Instances)
- **정의**: 특정 기간(1년 또는 3년) 동안 인스턴스를 예약하여 할인된 요금으로 사용하는 방식.
- **특징**:
  - 장기 약정을 통해 최대 75% 비용 절감.
  - 예약된 인스턴스를 사용하지 않을 때도 비용이 발생.
  - 표준 예약 인스턴스, 가변 예약 인스턴스, 예약 용량 중 선택 가능.
- **사용 사례**:
  - 일관된 사용 패턴을 가진 애플리케이션.
  - 장기적인 예산 계획이 필요한 경우.
  - 미션 크리티컬 애플리케이션.

### 4. 세이빙 플랜 (Savings Plans)
- **정의**: 사용량에 기반하여 특정 기간(1년 또는 3년) 동안 약정하여 할인된 요금을 제공하는 방식.
- **특징**:
  - 유연한 약정 방식으로 온디맨드 인스턴스와 유사하게 사용 가능.
  - Compute Savings Plans와 EC2 Instance Savings Plans로 구분.
- **사용 사례**:
  - 유연성이 필요하지만 비용 절감도 중요한 경우.
  - 여러 인스턴스 유형 및 리전에서 사용량을 최적화하고자 할 때.

### 5. 전용 호스트 (Dedicated Hosts)
- **정의**: 단일 고객 전용 물리적 서버를 제공하여 라이선스 관리 및 컴플라이언스 요구 사항을 충족할 수 있도록 하는 방식.
- **특징**:
  - 물리적 서버를 전용으로 사용 가능.
  - 하드웨어 수준의 제어와 가시성 제공.
  - 기존 소프트웨어 라이선스를 사용할 수 있도록 지원.
- **사용 사례**:
  - 라이선스 제약이 있는 소프트웨어 사용.
  - 물리적 서버에 대한 제어와 가시성이 필요한 워크로드.
  - 데이터 센터 통합 및 마이그레이션.

### 6. 전용 인스턴스 (Dedicated Instances)
- **정의**: 전용 물리적 서버에서 실행되는 인스턴스로, 다른 고객의 인스턴스와 하드웨어를 공유하지 않음.
- **특징**:
  - 전용 하드웨어에서 인스턴스 실행.
  - 다른 AWS 고객과 물리적 호스트를 공유하지 않음.
  - 온디맨드 및 예약 인스턴스와 함께 사용 가능.
- **사용 사례**:
  - 물리적 서버에 대한 격리와 보안이 중요한 워크로드.
  - 특정 컴플라이언스 요구 사항이 있는 경우.

### 요약
- **온디맨드 인스턴스**: 유연하고 예측 불가능한 워크로드에 적합.
- **스팟 인스턴스**: 비용 절감이 중요한 유연한 워크로드에 적합.
- **예약 인스턴스**: 장기적으로 일관된 사용 패턴이 있는 애플리케이션에 적합.
- **세이빙 플랜**: 다양한 인스턴스 유형과 리전에 유연하게 적용 가능한 비용 절감 옵션.
- **전용 호스트 및 전용 인스턴스**: 물리적 서버 제어와 특정 컴플라이언스 요구 사항이 있는 경우에 적합.
...
### 개요
- **정의**: 완전 관리형 메시지 대기열 서비스로, 분산된 애플리케이션 구성 요소 간에 메시지를 송수신할 수 있도록 합니다.
- **특징**:
  - **표준 대기열(Standard Queue)**: 무제한 처리량을 제공하며, 메시지가 적어도 한 번 이상 전달되지만, 중복될 수 있습니다. 메시지 순서가 보장되지 않습니다.
  - **FIFO 대기열(FIFO Queue)**: 메시지의 중복 없는 정확한 1회 전달 및 메시지 순서 보장을 제공. 처리량이 제한적입니다.

### 주요 개념
- **메시지**: 대기열에 저장되는 데이터 블록으로, 본문, 메타데이터, 속성 등을 포함.
- **생산자(Producer)**: 메시지를 대기열에 전송하는 엔티티.
- **소비자(Consumer)**: 대기열에서 메시지를 읽는 엔티티.
- **Visibility Timeout**: 메시지가 소비자에 의해 읽혔지만 삭제되기 전까지 다른 소비자에게 보이지 않도록 하는 시간.

### 사용 사례
- 비동기 작업 처리
- 주문 처리 시스템
- 로깅 및 모니터링 시스템에서 로그 수집

## Amazon Simple Notification Service (SNS)

### 개요
- **정의**: 대규모 메시징 서비스로, 애플리케이션 간에 메시지를 주고받을 수 있도록 합니다. 주로 이벤트 기반 아키텍처에서 사용됩니다.
- **특징**:
  - **주제(Topic)**: 메시지를 전송할 수 있는 엔드포인트. 주제에 구독된 모든 엔드포인트로 메시지가 전송됩니다.
  - **구독(Subscription)**: 주제에 메시지를 수신할 엔드포인트를 등록하는 작업. HTTP/S, Email, SQS, Lambda 등이 지원됩니다.

### 주요 개념
- **주제(Topic)**: 메시지를 게시하는 엔드포인트.
- **구독자(Subscriber)**: 주제에 메시지를 수신하는 엔드포인트.
- **메시지 게시(Publish)**: 주제에 메시지를 전송하는 작업.
...
### 개요
- **정의**: 실시간 데이터 스트리밍 서비스로, 대규모 스트리밍 데이터를 수집, 처리, 분석할 수 있도록 합니다.
- **구성 요소**:
  - **Kinesis Data Streams**: 실시간으로 데이터를 스트리밍하고 처리할 수 있는 서비스.
  - **Kinesis Data Firehose**: 스트리밍 데이터를 S3, Redshift, Elasticsearch 등으로 실시간으로 로드.
  - **Kinesis Data Analytics**: 스트리밍 데이터를 실시간으로 분석할 수 있는 SQL 기반의 서비스.

### 주요 개념
- **샤드(Shard)**: 데이터 스트림 내에서 데이터를 저장하는 기본 단위로, 각각의 샤드는 처리할 수 있는 처리량이 제한적입니다.
- **프로듀서(Producer)**: 데이터를 스트림으로 전송하는 엔티티.
- **컨슈머(Consumer)**: 스트림에서 데이터를 읽는 엔티티.
- **레코드(Record)**: 스트림에서 단위 데이터를 나타내는 엔티티.
...
### 주요 차이점 요약
- **SQS**: 메시지 대기열 서비스로, 비동기 작업 처리 및 메시지 순서가 중요하지 않은 애플리케이션에 적합.
- **SNS**: 메시지 전송 및 이벤트 기반 아키텍처에서 주제를 통해 여러 구독자에게 메시지를 전달하는 서비스.
- **Kinesis**: 실시간 데이터 스트리밍 및 분석을 위한 서비스로, 대규모 스트리밍 데이터를 처리하고 분석하는 데 적합.
